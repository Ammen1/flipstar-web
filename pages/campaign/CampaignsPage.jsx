...
export function CampaignsPage({ onCampaignClick, onBack }) {
  ...
  // Handle selected campaign from sessionStorage
  useEffect(() => {
    const selectedCampaignId = sessionStorage.getItem('selectedCampaignId');
    if (selectedCampaignId) {
      console.log('[CampaignsPage] Opening campaign detail for ID:', selectedCampaignId);
      // Open the campaign detail modal
      setSelectedCampaign(campaigns.find(c => c.id === parseInt(selectedCampaignId)));
      // Clear the stored ID
  ...
  const getActualStatus = (campaign) => {
    // A campaign is only "ended" when its entry deadline has passed
    const deadline = campaign.entry_deadline || campaign.end_date;
    if (deadline) {
      const diff = new Date(deadline) - new Date();
      if (diff <= 0) return 'completed';
    }
    // voting_end alone does NOT mark a campaign as ended if entries are still open
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{
          position: 'relative',
          textAlign: 'center',
          margin: isMobile ? '12px 0 16px' : '8px 0 24px',
          ...
          overflow: 'hidden',
            ...
            <h1 style={{ margin: 0, fontSize: isMobile ? 22 : 26, fontWeight: 900, color: T.txt, letterSpacing: '-0.02em' }}>
              Win Real Prizes
            </h1>
            <p style={{ margin: '6px 0 0', fontSize: isMobile ? 12 : 13, color: T.sub, maxWidth: 480, marginInline: 'auto' }}>
          ...
          {loading ? (
            ...
            <div style={{
              ...
              {campaigns.map((campaign, idx) => {
                ...
                const meta = STATUS_META[actualStatus] || STATUS_META.completed;
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        {getTimeRemaining(campaign.entry_deadline || campaign.voting_end)}
                      </div>
                    </div>
