...
const resolveMediaUrl = (url) => {
  ...
  // For relative paths, construct absolute URL using API base
  const root = config.API_BASE_URL.replace('/api', '');
  return `${root}${url}`;
};
    ...
    >
      <div style={{ position: 'relative', maxWidth: '78%' }}>
        {(() => {
          ...
          // Plain text with shared post preview
          return (
            <div>
              {sharedPostId && (
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${own ? 'rgba(255,255,255,0.3)' : 'rgba(143,196,65,0.3)'}`,
                    borderRadius: 12,
                    marginBottom: 8,
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = own ? 'rgba(255,255,255,0.22)' : 'rgba(143,196,65,0.15)';
                  }}
                  onMouseLeave={(e) => {
              ...
              {!messageWithoutPostId && !sharedPostId && (
                <div style={{
                  padding: '10px 14px',
                  fontSize: 14, lineHeight: 1.4,
                  wordBreak: 'break-word', whiteSpace: 'pre-wrap',
