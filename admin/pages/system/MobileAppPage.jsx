...
import { Smartphone, Bell, RefreshCw, ToggleLeft, ToggleRight, Users, Activity, Send, CheckCircle, XCircle, Zap, Download, Globe, Settings, BarChart3 } from 'lucide-react';
import api from '../../../api';
...
export function MobileAppPage({ theme: T }) {
  ...
  const tabs = [
    ...
    { id: 'update', label: 'Force Update', icon: RefreshCw },
  ];
  ...
  const Toggle = ({ value, onChange }) => (
    <button
      ...
      style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: value ? (T?.orange || T?.warning || '#8fc441') : (T?.sub || '#aaa') }}
    >
      {value
        ? <ToggleRight size={28} />
        : <ToggleLeft  size={28} />
  ...
  return (
    <div style={{ maxWidth: 1200 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: (T?.orange || T?.warning || '#8fc441') + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', color: T?.orange || T?.warning || '#8fc441' }}>
          <Smartphone size={22} />
        </div>
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: T?.txt }}>Mobile App Management</h1>
      ...
      <div style={{
        ...
        {tabs.map(tab => (
          <button
            ...
            style={{
              padding: '12px 20px',
              background: activeTab === tab.id ? (T?.orange || T?.warning || '#8fc441') + '20' : 'transparent',
              color: activeTab === tab.id ? (T?.orange || T?.warning || '#8fc441') : T?.sub,
              border: `1px solid ${activeTab === tab.id ? (T?.orange || T?.warning || '#8fc441') : 'transparent'}`,
              borderRadius: 8,
              cursor: 'pointer',
              fontSize: 14,
              fontWeight: 600,
      ...
      {activeTab === 'overview' && (
        <div style={card}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
            ...
            {statCard('Active (30 d)',   stats?.users?.active_month,  Activity, T?.orange || T?.warning || '#8fc441')}
      ...
      {activeTab === 'push' && (
        <div style={card}>
          <h2 style={{ ...h2, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Bell size={18} color={T?.orange || T?.warning || '#8fc441'} /> Push Notifications
          </h2>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: sending ? '#ccc' : (T?.orange || T?.warning || '#8fc441'),
      ...
      {activeTab === 'features' && (
        <div style={card}>
          <h2 style={{ ...h2, display: 'flex', alignItems: 'center', gap: 8 }}>
            <ToggleRight size={18} color={T?.orange || T?.warning || '#8fc441'} /> Mobile Feature Flags
          </h2>
          ...
          <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
            <button
              ...
              style={{
                ...
                background: T?.orange || T?.warning || '#8fc441', color: '#fff', fontWeight: 700, fontSize: 15,
                cursor: 'pointer', boxShadow: `0 4px 16px ${(T?.orange || T?.warning || '#8fc441')}55`,
              }}
            >
              Save Features
            </button>
      ...
      {activeTab === 'update' && (
        <div style={card}>
          <h2 style={{ ...h2, display: 'flex', alignItems: 'center', gap: 8 }}>
            <RefreshCw size={18} color={T?.orange || T?.warning || '#8fc441'} /> Force Update Banner
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
