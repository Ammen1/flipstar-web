...
export function EditCampaignModal({ theme, campaign, onClose, onSuccess, selectedMasterCampaign }) {
  ...
  const [prizeType, setPrizeType] = useState(
    (parseFloat(campaign.prize_value) === 0 && campaign.prize_title?.toLowerCase().includes('data'))
      ? 'data'
      : 'birr'
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('prize_value',        prizeType === 'data' ? '0' : formData.prize_value);
  ...
  const dateInp = dateInputStyle(theme);
        ...
        <div style={{ padding: '24px 32px' }}>
          ...
          <Section icon={Trophy} title="Prize Details" color={theme.green}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <Field label="Prize Type" required theme={theme}>
                <select
                  value={prizeType}
                  onChange={(e) => {
                    ...
                    if (e.target.value === 'data') {
                      set('prize_value', '0');
                      if (!formData.prize_title) set('prize_title', '1 GB Data');
                    } else {
                      set('prize_value', '');
                    }
                  }}
                  style={inp}
                >
                  <option value="birr">Birr (Cash)</option>
                  <option value="data">1 GB Data</option>
                </select>
              </Field>
            </div>
            {prizeType === 'birr' && (
            <Field label="Amount (Birr)" required theme={theme}>
