...
import { Trophy, Clock, Zap, ChevronRight, Award, Calendar, CheckCircle } from 'lucide-react';
import api from '../../api';
...
function ActiveCard({ c, T, onJoin, isJoined, onClick }) {
  return (
    <div
      onClick={() => onClick?.(c)}
      style={{
    ...
    >
      ...
      <div style={{ padding: '10px 12px 12px' }}>
        ...
        >
          {isJoined ? <Award size={12} /> : <Zap size={12} />}
          {isJoined ? 'Joined' : 'Join Now'}
          {!isJoined && <ChevronRight size={12} />}
        </button>
      </div>
    </div>
  );
...
function UpcomingCard({ c, T, onJoin, onClick }) {
  return (
    <div
      onClick={() => onClick?.(c)}
      style={{
...
export function SidebarCampaigns({ onCampaignClick }) {
  ...
  // Don't read joined from cache - it could be stale from another user
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        console.log('SidebarCampaigns: Starting fetch...');
        const [a, u, w, j] = await Promise.all([
          ...
          api.request('/campaigns/profile/').catch((err) => {
            console.error('Failed to fetch campaign profile:', err);
            return { campaigns: [] };
          }),
        ]);
        console.log('[SidebarCampaigns] Campaign profile response:', j);
        ...
        // Backend returns { campaigns: [{ campaign: { id, ... } }] }
        // Normalize IDs to numbers for consistent comparison
        const joinedIds = Array.isArray(j?.campaigns) 
          ? j.campaigns.map(e => Number(e.campaign?.id)).filter(id => !isNaN(id)) 
          : [];
        console.log('[SidebarCampaigns] Joined campaign IDs:', joinedIds);
        console.log('[SidebarCampaigns] Active campaign IDs:', aList.map(c => c.id));
        
        // Sort active campaigns: unjoined first, then joined
        const sortedActive = [...aList].sort((a, b) => {
          const aJoined = joinedIds.includes(Number(a.id));
          const bJoined = joinedIds.includes(Number(b.id));
          if (aJoined === bJoined) return 0;
          return aJoined ? 1 : -1;
        });
        
        setActive(sortedActive);   scWrite('sc_active', sortedActive);
        setUpcoming(uList); scWrite('sc_upcoming', uList);
        setWinners(wList);  scWrite('sc_winners', wList);
        setJoinedCampaigns(joinedIds); scWrite('sc_joined', joinedIds);
      } catch (e) {
          ...
          {loading
            ...
            : active.map(c => (
                <div key={c.id} style={{ marginBottom: 10 }}>
                  <ActiveCard c={c} T={T} onJoin={onCampaignClick} isJoined={joinedCampaigns.includes(Number(c.id))} onClick={onCampaignClick} />
                </div>
              ))}
        </div>
      )}
          ...
          {loading
            ...
            : upcoming.map(c => (
                <div key={c.id} style={{ marginBottom: 8 }}>
                  <UpcomingCard c={c} T={T} onJoin={onCampaignClick} onClick={onCampaignClick} />
                </div>
              ))}
        </div>
      )}
