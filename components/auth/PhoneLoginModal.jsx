...
export function PhoneLoginModal({
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (e?.status === 429) {
        ...
        e?.data?.code === "pin_not_set" ||
        e?.data?.requires_pin_setup
      ) {
  ...
  const handleSuperappPhoneSubmit = async (e) => {
    ...
    try {
      ...
      const checkRes = await api.post("/subscription/check-superapp/", {
      ...
      if (!checkRes.data.has_active_subscription) {
        setSuperappError(
          "No active SuperApp subscription found. Please subscribe first.",
        );
      ...
      const otpRes = await api.post("/auth/resend-subscription-otp/", {
      ...
      setSuperappError(
        extractErrorMessage(e, "Failed to check subscription or send OTP"),
      );
    } finally {
      setSuperappLoading(false);
    }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      await api.post("/auth/resend-subscription-otp/", {
        phone: toE164(phone),
      });
    ...
    <>
      ...
      {activeModal === "superapp-phone" && (
        ...
        >
          ...
          >
            ...
            >
              <div style={{ fontSize: 20, fontWeight: 900, color: "#8fc441" }}>
                SuperApp Login
              </div>
              <button
                onClick={() => setActiveModal(null)}
            ...
            <form onSubmit={handleSuperappPhoneSubmit}>
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    ...
                    value={superappPhone}
                    onChange={(e) =>
                      setSuperappPhone(sanitizePhoneInput(e.target.value))
                    }
                    ...
                    onFocus={(e) =>
                      (e.target.style.border = "1.5px solid #8fc441")
                    }
                    onBlur={(e) =>
                      (e.target.style.border = "1.5px solid #262626")
                    }
                  />
                </div>
              </div>
              <button
                type="submit"
                disabled={superappLoading}
                style={{
                  width: "100%",
                  padding: "14px",
                  background: superappLoading ? "#3A3A3A" : GOLD,
                  border: "none",
                  borderRadius: 10,
                  color: superappLoading ? "#888" : "#000",
                  fontSize: 15,
                  fontWeight: 800,
                  cursor: superappLoading ? "not-allowed" : "pointer",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  gap: 8,
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "#ccc", marginBottom: 16 }}>
              ...
              {superappPhone.replace(/(\d{3})(\d{3})(\d{4})/, "$1 $2 $3")}
            </div>
            <form onSubmit={handleSuperappOtpSubmit}>
              <div style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  value={superappOtp}
                  onChange={(e) =>
                    setSuperappOtp(e.target.value.replace(/\D/g, ""))
                  }
                  placeholder="••••••"
                  style={{
                    ...inp(false),
      ...
      >
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          >
            <div
              style={{ textAlign: "center", marginBottom: isMobile ? 10 : 28 }}
            >
              <div
                style={{
